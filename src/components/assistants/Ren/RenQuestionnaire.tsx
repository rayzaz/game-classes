// src/components/assistants/Ren/RenQuestionnaire.tsx
...
export default function RenQuestionnaire({
  assistant = { name: "Рен" },
  classes,
  onCancel,
  onFinish,
  ...
  return (
    <div className="portal">
      ...
      {step === 1 && (
        <>
          <Hint>
            Начнём с самого простого — с твоего имени. Запиши его сюда. Теперь возраст: тебе должно быть от 13 до 15 лет, это время, когда подростки получают свои гримуары. Дальше место рождения — выбери одну из стран: Клевер, Алмаз, Пики или Червы. Ну и пара слов о биографии: семья, происхождение, учёба. Если ты из деревни, пиши о простом детстве, если из столицы — упор на образование. А если у тебя есть редкая родовая магия, укажи это обязательно!
          </Hint>
      ...
      {step === 2 && (
        <>
          <Hint>
            Теперь давай разберёмся с внешностью. Укажи рост и вес — это важно для боевой подготовки. Телосложение можешь выбрать: худощавый, средний или крепкий. Волосы — напиши цвет и длину, то же самое с глазами. А если есть особые приметы, вроде шрама, родинки или даже татуировки — тоже впиши. Эти детали помогают отличать тебя от других магов. Да и потом будет проще тебя искать, если вдруг потеряешься.
          </Hint>
      ...
      {step === 7 && (
        <>
          <Hint>
            Ну вот и последняя часть — система боя. Тут всё просто: в бою игрок и противник бросают кубы. К результату прибавляются бонусы от твоего класса, магии или предметов. Заклинания могут накладывать эффекты — ожог, лечение, щит, замедление и так далее. Итоговый урон или защита определяется суммой куба и твоих бонусов. Побеждает тот, у кого результат выше. Так что не забывай: твоя магия важна, но кубик тоже играет роль!
          </Hint>

...
type Props = {
  speaking: boolean;  // рот «хлопает», когда true
  idleSrc?: string;   // обычный кадр
  openSrc?: string;   // рот открыт (говорит)
  blinkSrc?: string;  // глаза закрыты (моргание)
  width?: number;
  height?: number;
};
...
export default function AssistantRen({
  ...
  React.useEffect(() => {
    ...
    const schedule = () => {
      const delay = 2500 + Math.random() * 2500; // 2.5–5 сек
